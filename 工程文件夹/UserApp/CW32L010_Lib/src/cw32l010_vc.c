...
//VC1输出滤波使能
void VC1_EnableFilter(VC_FilterTypeDef VC_FilterStruct)
{
    assert_param(IS_VC_FLT_CLK(VC_FilterStruct.VC_FilterClk));
    assert_param(IS_VC_FLT_TIME(VC_FilterStruct.VC_FilterTime));

    REGBITS_MODIFY(CW_VC1->CR1, VCx_CR1_FLTCLK_Msk | VCx_CR1_FLTTIME_Msk, \
                   VC_FilterStruct.VC_FilterClk | VC_FilterStruct.VC_FilterTime);
    
}
//VC2输出滤波使能
void VC2_EnableFilter(VC_FilterTypeDef VC_FilterStruct)
{
    assert_param(IS_VC_FLT_CLK(VC_FilterStruct.VC_FilterClk));
    assert_param(IS_VC_FLT_TIME(VC_FilterStruct.VC_FilterTime));

    REGBITS_MODIFY(CW_VC2->CR1, VCx_CR1_FLTCLK_Msk | VCx_CR1_FLTTIME_Msk, \
                   VC_FilterStruct.VC_FilterClk | VC_FilterStruct.VC_FilterTime);
...
{
    REGBITS_CLR(CW_VC1->CR1, VCx_CR1_FLTCLK_Msk | VCx_CR1_FLTTIME_Msk);
}
//VC2输出滤波禁用
void VC2_DisableFilter(void)
{
    REGBITS_CLR(CW_VC2->CR1, VCx_CR1_FLTCLK_Msk | VCx_CR1_FLTTIME_Msk);
}
...
FlagStatus VC1_GetFlagStatus(uint16_t VC_FLAG)
{
    ...
    assert_param(IS_VC_FLAG(VC_FLAG));

    if ((CW_VC1->SR & VC_FLAG))
    {
        bitstatus = SET;
    }
    else
