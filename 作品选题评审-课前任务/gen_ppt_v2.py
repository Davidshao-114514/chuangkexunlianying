# -*- coding: utf-8 -*-
"""AI 辅助选题评价（任务4）+ 选题答辩 PPT V2.0（任务5）—— 按 AI 评价迭代"""
import os
...
FNAME = os.path.join(OUT, "仿真跑不队_磁吸焊接辅助手电_选题答辩PPT_V2.0.pptx")
BLUE = C(31, 56, 100); DGRAY = C(60, 70, 90); LGRAY = C(130, 138, 152)
RED = C(200, 40, 40); GREEN = C(46, 160, 90)
...
def tit(s, t, sub=""):
    ...
    if sub: tx(s, 0.5, 0.9, 12.3, 0.42, sub, size=12, color=LGRAY)
...
def tab(s, x, y, w, rows, hrow=0.42, fsize=11):
...
def bullets(s, items, y=1.45, size=13):
    for it in items:
        g = 0.6 if len(it) < 85 else 0.62
        tx(s, 0.65, y, 12.2, 0.95, "· " + it, size=size, color=DGRAY)
        y += g

def note(s, t, y=6.72):
    tx(s, 0.6, y, 12.2, 0.55, t, size=11.5, color=RED)

# ============ V1.0 页面（含 V2.0 修改点：P5 竞品行、P12 Q&A 修正）
P1 = ("cover", "磁吸焊接辅助手电", "——\u201c能自己站住的第三只手\u201d——\n\n仿真跑不队（1 班 C 组）｜邵子中、李昊桐、姜亚楷\n\nAI+OPC 能力范式竞赛 · 2026 年 8 月 28 日 · V2.0（经 AI 评价迭代）")

P2 = ("page_tb", "选题背景：被忽视的焊台照明痛点",
      "数据来源：126 份问卷（2026-08，问卷星平台，95% CI ±8.7% 阶段值，已按 AI 评价补充分层抽样计划至 400 份收敛 ±5%）＋3 次深度访谈＋60 名焊接工人实景测试",
...
P5 = ("page_tab", "竞品分析：功能对比矩阵（量化）",
      "价位/功能比 = 功能覆盖得分 ÷ 价格（元）；数据来源：[2] BOM 实测/样机参数，[1] 问卷第 9-11 题，[3] AI 评价补充：米家磁吸灯（真实同形态竞品）",
      [["维度", "台灯（60-200 元）", "廉价便携灯（20-49 元）", "米家磁吸灯（29-49 元）", "专业机床灯（300+ 元）", "磁吸焊接手电（59-79 元）"],
       ["自由角度", "2 轴｜僵直", "无", "2 轴", "3 轴", "万向 360°（3 轴）[2]"],
       ["桌面占用", "25-40%", "0", "0", "40%+（夹装）", "0（吸附离场）[2]"],
       ["亮度控制", "2-3 档", "1-2 档", "3 档", "无级+聚光", "无级 20kHz 无频闪[2]"],
       ["状态可视", "无", "无", "无", "无", "电量+充电+OLED 扩展[2]"],
       ["稳定性", "—", "夹不稳（61.11%）", "磁吸（无万向）", "螺丝固定", "N42≥2.5kg、漂移<1°[2]"],
       ["价位/功能比", "0.6 分/元", "1.0", "1.1", "0.15", "1.6（性价比最高）"]],
      "定位结论：我们与米家磁吸灯的同形态差异=万向悬停+无级无频闪+专业级稳定性；与专业灯差异=价格 75%+ 降幅；与台灯/廉价灯差异=零占用+可瞄准。")
...
P9 = ("page_tb", "MVP 验证：用最低成本证明“值得做”",
      ...
      ["MVP 形态：无屏 PCBA 样机×2 + OLED 外接版×2，20 人 A/B 对比（已有 BOM 34 行 56 位号样机）",
       ...
       "基线对照组：沉浸式体验已测“无灯 50%→台灯 30%→手电 20%”失败率梯度（附录 B 数据索引），A/B 结论与基线互证",
       "成本效益：验证总成本 ≈¥300 VS 假设错误代价（OLED 集成 +¥5/台×300 台+2 周开发）——验证成本 <1% 错误成本",
...
P10 = ("page_tab", "增长潜力：TAM / SAM / SOM 三级测算", "PEST：职教实训政策扶持、DIY 市场年增 15%、焊接技能热潮、低成本传感+OPC UA 普及",
      ...
      "规模来源：教育部高校学科统计+电赛报名数据+DIY 社区公开数据（2026-08 估算）；注：AI 评价要求标注“估算性质”，已注明来源与口径")
...
P12 = ("page_tb", "Q&A 预判：最尖锐的 3 个问题",
      "问题来源：精益画布自检识别的核心缺陷 ＋ 六顶思考帽黑帽风险 ＋ AI 评价补充（已按 AI 评价修正第 3 问矛盾）",
      ["Q1“材料都是标准件，30 天就能被仿制”——应对：真壁垒是基地“老带新”渠道（年 300+ 新生触达）+ 万向悬停专利（申请中）；供应链锁定只是进度优势，自评已弱化该项",
       ...
       "Q3“学生会自费 59-79 元吗”——应对：专业工具刚需场景；61.11% 廉价灯不合格率是支付意愿锚点；H-001 虚拟支付 30 人实测回应。若失守将至 39 元档——【修正】39 元档成本测算：3D 打印外壳+无 OLED 扩展版 BOM ≈27 元，毛利 30.8%（与 H-005 的 40% 目标不矛盾：40% 对应标准版 79 元）",
       "实验失败预案（来自失败标准）：H-001 失效→青春版 39 元+DIY 套件引流；H-002 失效→OLED 集成或提示音替代；H-005 失效→结构件 3D 打印降级"])

# ============ V2.0 新增：AI 评价与团队分析（2 页）
P13 = ("page_tb", "AI 评价与团队分析（1/2）：提示词与完整评价",
      "AI 工具：DeepSeek（Hermes Agent 桌面版）｜提问日期：2026-08-28｜前置输入：PPT V1.0 完整文件（14 页）上传",
      ["提示词（节选）：这是我们的创客竞赛选题汇报 PPT，请从①需求真实性②方案可行性③竞争差异化④逻辑自洽性⑤风险与漏洞⑥证据可信度六个维度评价，提出至少 3 个尖锐、具体、可操作的问题或建议。",
       "AI 评价（6 维摘录）：④逻辑自洽——Q&A 页“降 39 元档保毛利 30%”与 H-005“毛利 ≥40%”矛盾：39 元价位需 BOM 压到 27 元，V1.0 无利润结构支撑；③竞争差异化——矩阵未列入米家磁吸灯（29-49 元，同为磁吸形态）；①需求真实性——问卷与前测样本全部来自基地圈层，自选择偏差（无普通用户对照）……（完整评价见答辩记录附件）",
       "AI 还指出：20 人 A/B 无“无灯/台灯”对照组基线；OPC UA 远期收入缺乏联网样机证据；“功能覆盖得分”为自拟权重无第三方依据。"])

P14 = ("page_tb", "AI 评价与团队分析（2/2）：逐条回应",
      "选取 AI 评价中最具代表性的 3 条：认同 / 已执行 / 存疑",
      ["【认同】样本自选择偏差——AI 指出问卷对象全部为基地/圈层人群，可信但窄。实际影响：我们已在任务 1 调研设计标准化中补充分层抽样（70% 学生/20% DIY/10% 工程师，补充至 400 份，误差收敛 ±5%），并新增 2 名非技术背景实训学生访谈样本。",
       "【已执行】（基于 AI 评价的具体修改，共 3 处）：①竞品矩阵新增“米家磁吸灯 29-49 元”列——此前矩阵漏掉同形态真实竞品；②修正 Q3 毛利率矛盾：39 元档追加 BOM 测算（3D 打印壳+无 OLED 扩展 =27 元/毛利 30.8%），与 H-005 的 40%（标准版 79 元）不再冲突；③增长潜力页标注 TAM 为“估算性质+来源口径”。",
       "【存疑】AI 认为“A/B 缺对照组”——我们不认同：附录 B 的沉浸式体验已提供基线（无灯 50%→台灯 30%→手电 20% 失败率梯度，2026-08-20），A/B 结论与此基线互证；AI 未读取附录数据索引即下此结论，属于信息不完整归因。对应计划：V2.0 页 9 已恢复“基线对照组”引用；若评委质疑，以附录 B 记录原件回应。"])
...
A2 = ("page_tb", "附录 B：数据来源与访谈记录索引（三可追溯）",
      ...
      ["问卷：126 份（2026-08-14~17，问卷星，95% CI ±8.7% 阶段值）→《磁吸焊接手电调研问卷数据》",
       ...
       "沉浸式体验：20 颗 0603 失败率梯度 50%→30%→20%（基线对照）",
...
def build(page):
    ...
    if kind == "cover":
        ...
        tx(s, 0.8, 5.95, 11.7, 0.7, "V2.0 · 作品选题汇报（含 AI 评价与团队分析）| 创客训练营·作品选题评审单元 | 口头汇报 8 分钟", size=12, color=LGRAY)
        return
    if kind == "page_tb":
        _, t, sub, items = page
        tit(s, t, sub)
        bullets(s, items, size=12.5)
        return
    if kind == "page_tab":
        if len(page) == 6:
...
for page in [P1, P2, P3, P4, P5, P6, P7, P8, P9, P10, P11, P12, P13, P14, A1, A2]:
    build(page)

prs.save(FNAME)
print("PPT V2.0 saved:", FNAME, "| 页数:", len(prs.slides._sldIdLst))
