 ...
 * 正常运行时作按键中断(未启用); 停机模式下为唤醒中断源, 唤醒后清零标志即可
 */
void GPIOA_IRQHandler(void)
{
    /* 清除所有已置位的中断标志 (写0清零, 见 GPIOA_INTFLAG_CLR) */
    if (CW_GPIOA->ISR != 0)
 ...
 * 1ms 周期中断: 作为系统时基, 驱动按键扫描/ADC采样/界面刷新等周期任务
 */
void BTIM1_IRQHandler(void)
{
    /* 确认 BTIM1 更新中断 */
    if (BTIM_GetITStatus(CW_BTIM1, BTIM_IT_UPDATE) == SET)
    {
        /* 清中断标志 */
        BTIM_ClearITPendingBit(CW_BTIM1, BTIM_IT_UPDATE);

        /* 周期任务(在 main.c 中实现, 内部按 1ms/5ms/50ms/200ms 分频调度) */
        App_Task1ms();
    }
}
